...
- For Vision SDK application initialization, call the global initialization function [MxInit()](./api/cpp/initialization_and_deinitialization.md#mxinit) before you call any related APIs. This function allocates device resources and log resources.
- If your application involves operator APIs, you can use [MxInitFromConfig()](./api/cpp/initialization_and_deinitialization.md#mxinitfromconfig) for global initialization. This function loads device and log resources from the operator configuration file and preloads the related operators at the same time, which improves API execution efficiency.
- If you need to configure global variables, such as adjusting the VPC channel resource pool size, call [MxInit(const AppGlobalCfg &globalCfg)](./api/cpp/initialization_and_deinitialization.md#mxinit) and pass in configuration parameters.

**After all Vision SDK APIs finish running**, call `MxDeInit()` to deinitialize the global resources that were initialized.

For details about the APIs, see [API Reference (C++)](./api/cpp/README.md).
...
For details about the related APIs, see [Custom Memory Resource Pool Management](./api/cpp/customized_memory_resource_pool_management.md#custom-memory-resource-pool-management).
...
Vision SDK uses synchronous execution by default. Some APIs support asynchronous execution by creating an `AscendStream`. For details about which APIs support asynchronous execution, see [API Reference (C++)](./api/cpp/README.md).

For details about the related APIs, see [Asynchronous Calls](./api/cpp/asynchronous_invocation.md#asynchronous-invocation).
...
You can implement video encoding by constructing a `VideoEncoder` instance. For configuration items, constraints, and supported capabilities, see the data structure description in [VideoEncodeConfig](./api/cpp/data_structures_and_enumeration_types.md#videoencodeconfig).

Video encoding supports a custom output data format. You can pass the encoding configuration through a custom callback function, which makes the encoded data easier to use. For details, see [VideoEncodeCallBack](./api/cpp/data_structures_and_enumeration_types.md#videodecodecallback).

For details about the video encoding APIs, see [VideoEncoder](./api/cpp/media_data_processing.md#videoencoder).
...
You can implement video decoding by constructing a `VideoDecoder` instance. For configuration items and their constraints and support details, see [VideoDecodeConfig](./api/cpp/data_structures_and_enumeration_types.md#videodecodeconfig).

Video decoding supports a custom output data format. You can pass the decoding configuration through a custom callback function, which makes the decoded data easier to use. For details, see [VideoDecodeCallBack](./api/cpp/data_structures_and_enumeration_types.md#videodecodecallback).

For details about the video decoding APIs, see [VideoDecoder](./api/cpp/media_data_processing.md#videodecoder).
...
For details about the API, see [Decode](./api/cpp/media_data_processing.md#decode).
...
For details about the API, see [Encode](./api/cpp/media_data_processing.md#encode).
...
For details about the API, see [Crop](./api/cpp/media_data_processing.md#crop).
...
For details about the API, see [Resize](./api/cpp/media_data_processing.md#resize).
...
For details about the API, see [Padding](./api/cpp/media_data_processing.md#padding).
...
For details about the API, see [CropResize](./api/cpp/media_data_processing.md#cropresize).
...
For details about the API, see [CropAndPaste](./api/cpp/media_data_processing.md#cropandpaste).
...
For details about the API, see [ConvertFormat](./api/cpp/media_data_processing.md#convertformat).
...
For details about the API, see [Crop](./api/cpp/media_data_processing.md#ZH-CN_TOPIC_0000001860120881).
...
For details about the API, see [Resize](./api/cpp/media_data_processing.md#ZH-CN_TOPIC_0000001813361448).
...
For details about the API, see [CropResize](./api/cpp/media_data_processing.md#ZH-CN_TOPIC_0000001813361304).
...
For details about the API, see [CvtColor](./api/cpp/media_data_processing.md#cvtcolor).
...
For details about the related APIs, see [TensorOperations](./api/cpp/media_data_processing.md#tensoroperations).
...
For details about specific APIs, see [TensorOperations](./api/cpp/media_data_processing.md#tensoroperations).
...
For details about the related APIs, see [Model](./api/cpp/model_inference.md#ZH-CN_TOPIC_0000001860000893).
...
2. Initialize the model.
    ...
    - Specify the loading method through the `loadType` field in `ModelLoadOptV2`, and then pass it to the `Model` API. This loading method distinguishes whether the model is loaded from a file or from memory, and whether the memory is managed internally by the system or by the user. For details, see [ModelLoadOptV2](./api/cpp/data_structures_and_enumeration_types.md#modelloadoptv2).
...
For details about the related APIs, see [Model Postprocessing](./api/cpp/model_postprocessing.md#model-postprocessing).
...
For details about the API, see [crop (single image cropping)](./api/python/media_data_processing.md#ZH-CN_TOPIC_0000001860120601) or [crop (batch cropping)](./api/python/media_data_processing.md#ZH-CN_TOPIC_0000001813200812).
...
Data passed between plugins consistently uses protobuf definitions. For the specific data format, see [Metadata](./api/cpp/data_structures_and_enumeration_types.md#metadata) and [Metadata proto file](./appendix.md#metadata-proto-files). Input data is parsed through protobuf, and output results are assembled through protobuf.
...
Users implement service logic in the `process` interface and use the interfaces provided in the [Base Component Layer](./api/cpp/basic_component_layer.md#basic-component-layer) as needed.

#### Sending Output Results

1. Build the output data structure.

    Data passed between plugins consistently uses protobuf definitions. Based on the protobuf data format provided in the API, select an appropriate structure. For the specific data format, see [Metadata](./api/cpp/data_structures_and_enumeration_types.md#metadata) and [Metadata proto file](./appendix.md#metadata-proto-files). If no match exists, users can define a custom structure, but they must follow the rules below.
...
>- `GetResultSP` returns smart-pointer data. Users do not need to manage this memory. For details, see [GetResultSP](./api/cpp/process_orchestration.md#getresultsp).
...
>- `GetResultWithUniqueIdSP` returns smart-pointer data. Users do not need to manage this memory. For details, see [GetResultWithUniqueIdSP](./api/cpp/process_orchestration.md#getresultwithuniqueidsp).
...
>- `GetMultiResultWithUniqueIdSP` returns smart-pointer data. Users do not need to manage this memory. For details, see [GetMultiResultWithUniqueIdSP](./api/cpp/process_orchestration.md#getmultiresultwithuniqueidsp).
