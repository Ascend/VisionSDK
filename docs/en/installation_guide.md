...
For the required dependency names, recommended versions, and acquisition suggestions in the CentOS environment, see [Table 1](#table20540329125614).
...
6. Go to the package upload path and install Vision SDK by using the following command. For constraints related to the installation path, see the description of `--install-path` in [Table 1](#table1361972315353).
...
|--quiet \| -q|Enable silent mode. Use it together with `--install` or `--upgrade`.|
...
3. Use the package upgrade command to upgrade the current Vision SDK package. See the following example for the upgrade command. For parameter details, see [Table 1](#table568416506475).
    ...
    **Table 1** Parameters and descriptions for Vision SDK package upgrade command
    ...
    |--quiet|Enable silent mode. Use it together with `--install` or `--upgrade`.|
