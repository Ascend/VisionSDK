...
2. 准备模型后处理，可参见[模型支持参考](./09.appendix.md#模型支持参考)使用Vision SDK提供的模型后处理。
...
插件之间传递的数据统一采用protobuf定义，具体的数据格式参考[Metadata](./api/cpp/data_structures_and_enumeration_types.md#metadata)和[Metadata proto文件](./09.appendix.md#metadata-proto文件)。相应的输入数据通过protobuf解析，输出结果通过protobuf组装。
...
1. 构建输出数据结构。
    ...
    插件之间传递的数据统一采用protobuf定义，根据API中提供的protobuf数据格式，选择合适的结构，具体的数据格式参考[Metadata](./api/cpp/data_structures_and_enumeration_types.md#metadata)和[Metadata proto文件](./09.appendix.md#metadata-proto文件)。无法匹配时，用户可自定义结构，但是必须遵守以下规则。
