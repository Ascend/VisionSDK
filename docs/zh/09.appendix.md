...
>[NOTE]
>若此表格中有“模型代码/文件获取地址”无法成功跳转，请前往源文件[VisionSDK文档-附录章节（开源社区）](https://gitcode.com/Ascend/VisionSDK/blob/master/docs/zh/09.appendix.md)的Code页面复制对应链接下载，或手动将无效链接中的`%2520`或`%20`替换为空格后再访问。
