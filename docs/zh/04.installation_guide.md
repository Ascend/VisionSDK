...
|依赖名称|版本建议|获取建议|
|--|--|--|
|gcc|7.3.0|建议通过获取源码包编译安装，安装步骤可参考[安装7.3.0版本gcc](09.appendix.md#安装730版本gcc)。|
...
|Python|3.9/3.10/3.11/3.12|建议通过获取源码包编译安装，安装步骤可参考[安装Python依赖](09.appendix.md#安装python依赖)。|
...
|protobuf|4.25.1|建议通过pip安装，安装命令参考如下。<br>```pip3 install protobuf==4.25.1```|
|Python|3.9|建议通过获取源码包编译安装，安装步骤可参考[安装Python依赖](09.appendix.md#安装python依赖)。|
...
安装完成后，可以通过以下方式快速验证 Vision SDK 是否安装成功。
...
# 检查 Vision SDK 安装版本（pip 安装方式）
pip3 show visionsdk

# 检查 Vision SDK 环境变量（run 包安装方式）
echo ${MX_SDK_HOME}
```

### Python API 快速验证

执行以下 Python 脚本，验证 Vision SDK 的基本功能是否正常：
...
ret = base.mx_init()
if ret != 0:
    print("Vision SDK 初始化失败")
    exit(1)

# 获取版本信息
print("Vision SDK 初始化成功")

# 去初始化
base.mx_deinit()
print("Vision SDK 去初始化成功")
```
...
如果使用容器部署，请先按照[快速入门](./03.quick_start.md)中的步骤启动容器，然后执行上述验证步骤。

### 常见问题

- 如果 `npu-smi info` 命令执行失败，请检查 NPU 驱动是否正常安装。
- 如果 Python 导入 `mindx.sdk` 失败，请检查 Vision SDK 是否正确安装，或确认 Python 环境路径是否正确。
- 更多问题请参考 [FAQ](./07.faq.md)。
