                                   ...
                                   MxTools::MxpiVision& dstMxpiVision)
{
    ...
    {
        cv::Rect ori(startCol, startRow, endCol, endRow);
                             ...
                             cv::Mat& src, MxBase::MemoryData memoryDst)
{
    ...
    if (memorySrc.type == THREE)
    {
        if (visionData.datatype() == MxTools::MxpiDataType::MXPI_DATA_TYPE_FLOAT32)
        {
            src = cv::Mat(visionInfo.heightaligned(), visionInfo.widthaligned(), CV_32FC3, memoryDst.ptrData);
        }
        else
        {
...
std::vector<std::shared_ptr<void>> MxpiSamplePlugin::DefineProperties()
{
    ...
    auto descriptionMessageProSptr = std::make_shared<ElementProperty<string>>(
        ElementProperty<string>{STRING, "descriptionMessage", "message", "Description mesasge of plugin",
                                "This is MxpiSamplePlugin", "NULL", "NULL"});
    auto startRow = std::make_shared<ElementProperty<double>>(
        ElementProperty<double>{DOUBLE, "startRow", "startRow", "the start_row of crop image", 1, 0.0, 8192.0});
    auto startCol = std::make_shared<ElementProperty<double>>(
        ElementProperty<double>{DOUBLE, "startCol", "startCol", "the start_col of crop  image", 1, 0.0, 8192.0});
    auto Height = std::make_shared<ElementProperty<double>>(
        ElementProperty<double>{DOUBLE, "Height", "Height", "the Height of crop image", 256, 1.0, 8192.0});
    auto Width = std::make_shared<ElementProperty<double>>(
        ElementProperty<double>{DOUBLE, "Width", "Width", "the Width of crop image", 256, 1.0, 8192.0});
    auto height = std::make_shared<ElementProperty<float>>(
        ElementProperty<float>{FLOAT, "height", "height", "the height of image", 256, 1, 8192});
    auto width = std::make_shared<ElementProperty<float>>(
        ElementProperty<float>{FLOAT, "width", "width", "the width of image", 256, 1, 8192});
    auto fx = std::make_shared<ElementProperty<double>>(
        ElementProperty<double>{DOUBLE, "fx", "fx", "the fx ratio  of image", 0, 0, 1});
    auto fy = std::make_shared<ElementProperty<double>>(
        ElementProperty<double>{DOUBLE, "fy", "fy", "the fy ratio  of image", 0, 0, 1});
    auto outputDataFormat = std::make_shared<ElementProperty<string>>(
        ElementProperty<string>{STRING, "outputDataFormat", "outputDataFormat",
                                "the format of the output  RGB or BGR or YUV", "YUV", "NULL", "NULL"});
    ...
    auto interpolation = std::make_shared<ElementProperty<int>>(
        ElementProperty<int>{INT, "interpolation", "interpolation", "the interpolation  of image", 1, 0, 4});
    auto option = std::make_shared<ElementProperty<string>>(
        ElementProperty<string>{STRING, "option", "option", "OPTION of plugin", "resize", "NULL", "NULL"});
