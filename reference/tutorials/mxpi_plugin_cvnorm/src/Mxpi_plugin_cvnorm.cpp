...
APP_ERROR MxpiPluginCvnorm::Process(std::vector<MxpiBuffer*>& mxpiBuffer)
{
    ...
    {
        ErrorInfo_ << GetErrorInfo(APP_ERR_COMM_FAILURE, pluginName_) << "MxpiPluginCvnorm process is not implemented";
    ...
    if (ret != APP_ERR_OK)
    {
        LogError << GetErrorInfo(ret, pluginName_) << "MxpiPluginCvnorm gets inference information failed.";
    ...
    if (ret != APP_ERR_OK)
    {
        ErrorInfo_ << GetErrorInfo(ret, pluginName_) << "MxpiPluginCvnorm add metadata failed.";
        mxpiErrorInfo.ret = ret;
        mxpiErrorInfo.errorInfo = ErrorInfo_.str();
        SetMxpiErrorInfo(*buffer, pluginName_, mxpiErrorInfo);
        return ret;
...
std::vector<std::shared_ptr<void>> MxpiPluginCvnorm::DefineProperties()
{
    ...
    auto descriptionMessageProSptr = std::make_shared<ElementProperty<string>>(
        ElementProperty<string>{STRING, "descriptionMessage", "message", "Description mesasge of plugin",
                                "This is plugin for cvnorm func", "NULL", "NULL"});
