 ...
 * History: NA
 */
#ifndef MXBASE_ENCODECONFIGWITHATLAS800IA2_H
#define MXBASE_ENCODECONFIGWITHATLAS800IA2_H
#include "MxBase/E2eInfer/DataType.h"
#include "ResourceManager/DvppWrapper/DvppConfig/Config/EncodeConfig.h"

namespace MxBase {
class EncodeConfigForYUVAndYVUWithAtlas800IA2 : public EncodeConfig {
public:
...
class EncodeConfigForRBGAndBGRWithAtlas800IA2 : public EncodeConfig {
public:
    APP_ERROR EncodeCheck(const DvppDataInfo& inputData, uint32_t encodeLevel)
    {
        return checkPtr_->Check(inputData, encodeLevel, config_);
...
public:
    HeightAndWidthConstrainInfo inputShapeConstrainInfo_ = { .wMin = 32, .wMax = 4096, .hMin = 32, .hMax = 4096 };
    uint32_t encodeLevelMin_ = 1;
    uint32_t encodeLevelMax_ = 100;
    uint32_t rgbAndBgrChannel_ = 3;
    uint32_t yuvAndYvuChannel_ = 0;
