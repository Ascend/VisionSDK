 ...
 * History: NA
 */
#ifndef MXBASE_CROPPASTECONFIGWITHATLAS800IA2_H
#define MXBASE_CROPPASTECONFIGWITHATLAS800IA2_H
#include "MxBase/E2eInfer/DataType.h"
#include "ResourceManager/DvppWrapper/DvppConfig/Config/CropPasteConfig.h"

namespace MxBase {
class CropPasteConfigForYUVAndYVUWithAtlas800IA2 : public CropPasteConfig {
public:
...
public:
    ...
    CropPasteConstrainConfig config_ = {pasteImageShapeConstrain_, cropWMin_, cropHMin_, pasteAreaShapeConstrain_,
                                        nullptr, inputShapeConstrain_, commonConstrainInfo_};
};

class CropPasteConfigForRGBAndBGRWithAtlas800IA2 : public CropPasteConfig {
public:
    APP_ERROR CropPasteCheck(const std::vector<DvppDataInfo> &inputDataVec,
                             const std::vector<DvppDataInfo> &pasteDataVec,
                             std::vector<CropRoiConfig> &cropAreaVec,
                             std::vector<CropRoiConfig> &pasteAreaVec)
    {
        return checkPtr_->Check(inputDataVec, pasteDataVec, cropAreaVec, pasteAreaVec, false, config_);
    }
public:
