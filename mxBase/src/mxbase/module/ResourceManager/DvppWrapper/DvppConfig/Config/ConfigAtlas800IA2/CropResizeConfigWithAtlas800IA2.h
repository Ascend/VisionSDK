 ...
 * History: NA
 */
#ifndef MXBASE_CROPRESIZECONFIGWITHATLAS800IA2_H
#define MXBASE_CROPRESIZECONFIGWITHATLAS800IA2_H
#include "MxBase/E2eInfer/DataType.h"
#include "ResourceManager/DvppWrapper/DvppConfig/Config/CropResizeConfig.h"

namespace MxBase {
class CropResizeConfigForYUVAndYVUWithAtlas800IA2 : public CropResizeConfig {
public:
...
public:
    ...
    OutputConstrainInfo outputConstrainInfo_ = {.outputWidthAlign = 16, .outputHeightAlign = 2, .widthStrideRatio = 1,
                                                .heightStrideRatio = 1.f, .outputMemoryRatio = 1.5f};
    CommonConstrainInfo commonConstrainInfo_ = {.strideShapeConstrain = strideShapeConstrain,
                                                .widthStrideAlign = 16, .heightStrideAlign = 2,
                                                .memorySizeRatio = 1.5f};
...
class CropResizeConfigForRGBAndBGRWithAtlas800IA2 : public CropResizeConfig {
public:
    APP_ERROR CropResizeCheck(const DvppDataInfo &inputData,
                              CropRoiConfig &cropRect,
                              const ResizeConfig &resizeScale)
    {
        return checkPtr_->Check(inputData, cropRect, resizeScale, false, config_);
    }
    APP_ERROR GetOutputDataSize(DvppDataInfo& outputData, uint32_t& outputDataSize)
    {
        return ConfigUtils::GetOutputDataSize(outputData, outputDataSize, outputConstrainInfo_);
...
public:
    ...
    OutputConstrainInfo outputConstrainInfo_ = {.outputWidthAlign = 16, .outputHeightAlign = 2, .widthStrideRatio = 3,
