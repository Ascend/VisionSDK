...
static std::map<TensorDType, uint32_t> TensorDataTypeSizeMap = {{TensorDType::UINT8, SIZE_OF_UINT8},
...
static APP_ERROR CheckTensorHW(const Tensor &tensor, const Size &minTensorSize, const Size &maxTensorSize)
{
    ...
    bool isGray = srcShape.size() == SHAPE_DIMENSION_TWO ||
                  (srcShape.size() == SHAPE_DIMENSION_THREE && srcShape[SHAPE_DIMENSION_TWO] == GRAY_WIDTH_EXTEND);
                                  ...
                                  bool isCropResize = false)
{
    ...
    if (resizeW < MIN_TENSOR_WIDTH || resizeW > MAX_TENSOR_WIDTH)
    {
        LogError << "Get invalid resize width(" << resizeW << "), " << "which should be in the range of ["
                 << MIN_TENSOR_WIDTH << ", " << MAX_TENSOR_WIDTH << "]." << GetErrorInfo(APP_ERR_COMM_INVALID_PARAM);
        return APP_ERR_COMM_INVALID_PARAM;
    }
    if (resizeH < MIN_TENSOR_HEIGHT || resizeH > MAX_TENSOR_HEIGHT)
    {
        LogError << "Get invalid resize height(" << resizeH << "), " << "which should be in the range of ["
                 << MIN_TENSOR_HEIGHT << ", " << MAX_TENSOR_HEIGHT << "]." << GetErrorInfo(APP_ERR_COMM_INVALID_PARAM);
        return APP_ERR_COMM_INVALID_PARAM;
    }
    ...
    if (resizeH * shrinkingRatio < tensorH || resizeH > zoomInRatio * tensorH)
    {
        LogError << "The tensor height zoom ratio is out of range [1/32 , " << zoomInRatio
                 << "], Actual zoom ratio is [" << static_cast<float>(resizeH) / tensorH << "], tensor width is ["
                 << tensorW << "] and height is [" << tensorH << "], resize width is [" << resizeW
                 << "] and height is [" << resizeH << "]." << GetErrorInfo(APP_ERR_COMM_INVALID_PARAM);
        return APP_ERR_COMM_INVALID_PARAM;
    }
    if (resizeW * shrinkingRatio < tensorW || resizeW > zoomInRatio * tensorW)
    {
        LogError << "The tensor width zoom ratio is out of range [1/32 , " << zoomInRatio << "], Actual zoom ratio is ["
...
static APP_ERROR CheckInputParamsTensorDType(const Tensor &src, const Tensor &dst, bool isResize)
{
    ...
    {
        LogError << "Unsupported src or dst Datatype, Only support uint8 TensorDType "
                    "when format is rgba or on Atlas800IA2."
                 << GetErrorInfo(APP_ERR_COMM_INVALID_PARAM);
        return APP_ERR_COMM_INVALID_PARAM;
    }
...
static APP_ERROR CheckSrcTensor(const Tensor &src, AscendStream &stream, bool checkResizePaste = false,
                                bool isResize = false)
{
    if (src.IsEmpty())
    {
    ...
    Size minSize =
        Size(checkResizePaste ? MIN_RESIZE_PASTE_BACKGROUND_TENSOR_WIDTH : MIN_TENSOR_WIDTH, MIN_TENSOR_HEIGHT);
...
static APP_ERROR CheckInputParams(const Tensor &src, Tensor &dst, AscendStream &stream, bool isResizePaste = false,
                                  bool isResize = false)
{
...
static void FreeDataIfNotReuse(hi_vpc_pic_info &inputDesc, hi_vpc_pic_info &outputDesc, bool reuseInputTensor,
                               bool reuseOutputTensor)
{
...
static APP_ERROR SetVpcInputDesc(hi_vpc_pic_info &inputDesc, const Tensor &inputTensor, uint32_t channel,
                                 bool &reuseInputTensor)
{
    ...
    if (IsWidthAligned(inputTensor))
    {
        ...
        ret = aclrtMemcpy(inputDesc.picture_address, inputDesc.picture_buffer_size, inputTensor.GetData(),
                          inputDesc.picture_buffer_size, ACL_MEMCPY_DEVICE_TO_DEVICE);
        if (ret != APP_ERR_OK)
        {
            LogError << "Failed to copy memory for input tensor." << GetErrorInfo(ret, "aclrtMemcpy");
            DVPPMemoryFreeFunc(inputDesc.picture_address);
...
static APP_ERROR GenerateNewOutputTensor(const Tensor &src, Tensor &dst, std::vector<uint32_t> &newShape, bool isDvpp,
                                         uint32_t widthExtend)
{
    if (!dst.IsEmpty())
    {
    ...
    bool shapeWithThreeDim =
        src.GetShape().size() == HWC_SHAPE_SIZE && src.GetShape()[SHAPE_DIMENSION_TWO] == GRAY_WIDTH_EXTEND;
                                        ...
                                        const Size &resize, uint32_t channel, bool &reuseOutputTensor)
{
    ...
    else
    {
        ...
        if (ret != APP_ERR_OK)
        {
            LogError << "Failed to malloc dvpp memory for resize output tensor." << GetErrorInfo(ret);
            return APP_ERR_COMM_ALLOC_MEM;
        }
    }
    return ret;
...
static APP_ERROR SetResizeOutputTensor(const Tensor &src, Tensor &dst, const Size &resize, bool keepMargin)
{
    ...
    if (!dst.IsEmpty() && dst.IsWithMargin() && !keepMargin && dst.GetValidRoi().y1 > 0 && dst.GetValidRoi().x1 > 0)
    {
    ...
    if (!IsMemoryReusable(dst) || actualDstMemSize != dst.GetByteSize())
    {
        ...
        else if (dst.GetMemoryType() != MemoryData::MemoryType::MEMORY_DEVICE || actualDstMemSize != dst.GetByteSize())
        {
...
APP_ERROR DoVpcResize(hi_vpc_chn &chnId, hi_vpc_pic_info &resizeInputDesc, hi_vpc_pic_info &resizeOutputDesc,
                      const Interpolation interpolation)
{
    ...
    if (ret != APP_ERR_OK)
    {
        LogError << "Failed to get Vpc process result in Resize." << GetErrorInfo(ret, "hi_mpi_vpc_get_process_result");
                                    ...
                                    const Interpolation interpolation, bool keepMargin)
{
    ...
    if (!keepMargin && !reuseOutputTensor)
    {
        for (size_t h = 0; h < dst.GetValidRoi().y1; h++)
        {
            ...
            ret = aclrtMemcpy(
                static_cast<void *>(static_cast<uint8_t *>(dst.GetData()) + offsetTo), dst.GetValidRoi().x1 * channel,
                static_cast<void *>(static_cast<uint8_t *>(resizeOutputDesc.picture_address) + offsetFrom),
                dst.GetValidRoi().x1 * channel, ACL_MEMCPY_DEVICE_TO_DEVICE);
            if (ret != APP_ERR_OK)
            {
                FreeDataIfNotReuse(resizeInputDesc, resizeOutputDesc, reuseInputTensor, reuseOutputTensor);
                LogError << "Failed to malloc dvpp memory for Resize input image." << GetErrorInfo(ret, "aclrtMemcpy");
                return APP_ERR_ACL_BAD_COPY;
            }
        }
...
APP_ERROR AscendCResize(const Tensor &src, Tensor &dst, const Size &, const Interpolation interpolation,
                        AscendStream &stream)
{
...
APP_ERROR DvppResize(const Tensor &src, Tensor &dst, const Size &resize, const Interpolation interpolation,
                     bool keepMargin, AscendStream &)
{
    hi_vpc_chn chnId = 0;
...
static APP_ERROR ResizeStreamProcess(const Tensor &src, Tensor &dst, const Size &resize,
                                     const Interpolation interpolation, bool keepMargin, bool isDvpp,
                                     AscendStream &stream)
{
    ...
    else
    {
        ResizeCallbackParam *resizeCallbackParam =
            new ResizeCallbackParam{src, dst, resize, interpolation, keepMargin, stream};
                                  ...
                                  const Size &resize, ResizeExcuteVpc &resizeExcuteVpc)
{
    ...
    uint32_t interpolationCann = currInterpolation > 0 ? currInterpolation - 1 : 0;
    uint32_t channel = GetWidthExtend(src);
    if (SetVpcInputDesc(resizeExcuteVpc.resizeInputDesc, src, channel, resizeExcuteVpc.reuseInputTensor) != APP_ERR_OK)
    {
    ...
    if (SetResizeVpcOutputDesc(resizeExcuteVpc.resizeOutputDesc, dst, resize, channel,
                               resizeExcuteVpc.reuseOutputTensor) != APP_ERR_OK)
    {
        FreeDataIfNotReuse(resizeExcuteVpc.resizeInputDesc, resizeExcuteVpc.resizeOutputDesc,
                           resizeExcuteVpc.reuseInputTensor, resizeExcuteVpc.reuseOutputTensor);
        return APP_ERR_COMM_INVALID_PARAM;
    ...
    ret = hi_mpi_vpc_resize(resizeExcuteVpc.chnId, &resizeExcuteVpc.resizeInputDesc, &resizeExcuteVpc.resizeOutputDesc,
                            0, 0, interpolationCann, &resizeExcuteVpc.taskId, -1);
    if (ret != APP_ERR_OK)
    {
...
APP_ERROR ResizeRgbaWithGetProcessResult(const Tensor &src, const Tensor &dst, ResizeExcuteVpc &resizeProcess,
                                         bool keepMargin)
{
    uint32_t channel = GetWidthExtend(src);
    ...
    if (!keepMargin && !resizeProcess.reuseOutputTensor)
    {
        ...
        {
            size_t offsetFrom =
                h * DVPP_ALIGN_UP(resizeProcess.resizeOutputDesc.picture_width, DVPP_ALIGN_WIDTH) * channel;
            ...
            ret = aclrtMemcpy(static_cast<void *>(static_cast<uint8_t *>(dst.GetData()) + offsetTo),
                              ...
                              static_cast<void *>(
                                  static_cast<uint8_t *>(resizeProcess.resizeOutputDesc.picture_address) + offsetFrom),
                              dst.GetValidRoi().x1 * channel, ACL_MEMCPY_DEVICE_TO_DEVICE);
            if (ret != APP_ERR_OK)
            {
                FreeDataIfNotReuse(resizeProcess.resizeInputDesc, resizeProcess.resizeOutputDesc,
    ...
    FreeDataIfNotReuse(resizeProcess.resizeInputDesc, resizeProcess.resizeOutputDesc, resizeProcess.reuseInputTensor,
                       resizeProcess.reuseOutputTensor);
    return ret;
}

APP_ERROR ResizeRgbaProcessWithMemReuse(const std::vector<Tensor> &src, const std::vector<Tensor> &dst,
                                        const Size &resize, const Interpolation interpolation, bool keepMargin)
{
    APP_ERROR ret = APP_ERR_OK;
                                  ...
                                  const Interpolation interpolation, bool keepMargin, AscendStream &stream)
{
    ...
    else
    {
        ResizeRgbaCallbackParam *resizeCallbackParam =
            new ResizeRgbaCallbackParam{src, dst, resize, interpolation, keepMargin, stream};
        CallBack dvppCallback;
                                    ...
                                    std::vector<MxBase::Tensor> &splitDstVec, const Size &resize)
{
    ...
    for (uint32_t i = 0; i < RGBA_WIDTH_EXTEND; ++i)
    {
        ...
        splitDstVec[i] = MxBase::Tensor({resize.height, resize.width, 1}, MxBase::TensorDType::UINT8, src.GetDeviceId(),
                                        applyDstDvpp);
...
static APP_ERROR ResizeForRgba(const Tensor &src, Tensor &dst, const Size &resize, const Interpolation interpolation,
                               bool keepMargin, AscendStream &stream)
{
...
void CallBack::ResizeRgbaCallBackFunc(void *arg)
{
    ...
    ret = ResizeRgbaProcess(input->src, input->dst, input->resize, input->interpolation, input->keepMargin,
                            input->stream);
    delete input;
    if (ret != APP_ERR_OK)
    {
...
APP_ERROR Resize(const Tensor &src, Tensor &dst, const Size &resize, const Interpolation interpolation, bool keepMargin,
                 AscendStream &stream)
{
    ...
    ret =
        CheckResizeParam(src, resize, Size(src.GetShape()[SHAPE_DIMENSION_ONE], src.GetShape()[SHAPE_DIMENSION_ZERO]));
    if (ret != APP_ERR_OK)
    {
...
void CallBack::ResizeCallBackFunc(void *arg)
{
    ...
    ret = DvppResize(input->src, input->dst, input->resize, input->interpolation, input->keepMargin, input->stream);
    delete input;
    if (ret != APP_ERR_OK)
    {
...
static APP_ERROR CheckResizePasteRect(const MxBase::Tensor &background, MxBase::Rect &rect)
{
    ...
    {
        LogError << "Input rect param is invalid. Rect is {" << rect.x0 << ", " << rect.y0 << ", " << rect.x1 << ", "
                 << rect.y1 << "}" << GetErrorInfo(APP_ERR_COMM_INVALID_PARAM);
        return APP_ERR_COMM_INVALID_PARAM;
    }
...
static APP_ERROR GenerateNewOutputTensorWithSrcData(const MxBase::Tensor &src, MxBase::Tensor &dst,
                                                    std::vector<uint32_t> &newShape, bool isDvpp, uint32_t widthExtend,
                                                    const MxBase::Rect &validRect)
{
    APP_ERROR ret = APP_ERR_OK;
    ...
    if (isDvpp)
    {
        ...
        ResizePasteMemcpyParam resizePasteMemcpyParam = {
            maxHeight, maxWidth, originalWidth, widthExtend, newOutputTensor.GetData(), src.GetData()};
...
static APP_ERROR ResizePasteMemoryReusable(const MxBase::Tensor &src, MxBase::Tensor &dst, uint32_t &actualDstMemSize,
                                           Rect &validRect)
{
    APP_ERROR ret = APP_ERR_OK;
    if (!dst.IsEmpty() && IsMemoryReusable(dst) && actualDstMemSize == dst.GetByteSize())
...
static APP_ERROR ResizePasteNotMemoryReusable(const MxBase::Tensor &src, MxBase::Tensor &dst, bool &keepMargin,
                                              ResizePasteOutputShape &resizePasteOutputShape,
                                              uint32_t &actualDstMemSize, Rect &validRect)
{
    ...
    if (!IsMemoryReusable(dst) || actualDstMemSize != dst.GetByteSize())
    {
        if (keepMargin)
        {
            ...
            ret = GenerateNewOutputTensorWithSrcData(src, dst, newShape, true, resizePasteOutputShape.widthExtend,
                                                     validRect);
            if (ret != APP_ERR_OK)
            {
                 ...
                 actualDstMemSize != dst.GetByteSize())
        {
            ...
            ret = GenerateNewOutputTensorWithSrcData(src, dst, newShape, false, resizePasteOutputShape.widthExtend,
                                                     validRect);
            if (ret != APP_ERR_OK)
            {
                LogError << "Failed to GenerateNewOutputTensor in SetResizeOutputTensorMaster operation."
...
static APP_ERROR MallocResizePasteMem(hi_vpc_pic_info *pastedImageDesc, const MxBase::Tensor &pastedTensor,
                                      uint32_t widthExtend)
{
    // malloc dvpp memory for input tensor
    ...
    if (ret != APP_ERR_OK)
    {
        LogError << "Failed to malloc dvpp memory for ResizePaste input tensor." << GetErrorInfo(ret);
        return APP_ERR_COMM_ALLOC_MEM;
    }
    if (IsWidthAligned(pastedTensor))
    {
        ...
        ret = aclrtMemcpy(pastedImageDesc->picture_address, pastedImageDesc->picture_buffer_size,
                          pastedTensor.GetData(), pastedImageDesc->picture_buffer_size, ACL_MEMCPY_DEVICE_TO_DEVICE);
        if (ret != APP_ERR_OK)
        {
    ...
    else
    {
        // memory not align, copy to dvpp line by line.
        size_t maxHeight =
            pastedTensor.GetValidRoi().y1 == 0 ? pastedImageDesc->picture_height : pastedTensor.GetValidRoi().y1;
        size_t maxWidth =
            pastedTensor.GetValidRoi().x1 == 0 ? pastedImageDesc->picture_width : pastedTensor.GetValidRoi().x1;
        ResizePasteMemcpyParam resizePasteMemcpyParam = {maxHeight,
                                                         maxWidth,
                                                         pastedImageDesc->picture_width,
                                                         widthExtend,
                                                         pastedImageDesc->picture_address,
                                                         pastedTensor.GetData()};
        ret = ResizePasteMemcpyByLine(resizePasteMemcpyParam);
        if (ret != APP_ERR_OK)
        {
                                         ...
                                         bool &reuseInputTensor)
{
    APP_ERROR ret = APP_ERR_OK;
    uint32_t widthExtend;
    pastedImageDesc->picture_height = pastedTensor.GetValidRoi().y1 > 0 ? pastedTensor.GetValidRoi().y1
                                                                        : pastedTensor.GetShape()[SHAPE_DIMENSION_ZERO];
    pastedImageDesc->picture_width = pastedTensor.GetValidRoi().x1 > 0 ? pastedTensor.GetValidRoi().x1
                                                                       : pastedTensor.GetShape()[SHAPE_DIMENSION_ONE];
        ...
        pastedTensor.GetShape()[SHAPE_DIMENSION_TWO] == SHAPE_DIMENSION_THREE)
    {
        ...
        pastedImageDesc->picture_width_stride =
            DVPP_ALIGN_UP(pastedTensor.GetShape()[SHAPE_DIMENSION_ONE], DVPP_ALIGN_WIDTH) * SHAPE_DIMENSION_THREE;
    }
    else
    {
...
static APP_ERROR SetResizePasteOutputDesc(hi_vpc_pic_info &outputImageDesc, MxBase::Tensor dst, bool &reuseOutputTensor)
{
    ...
    if (dst.GetShape().size() == HWC_SHAPE_SIZE && dst.GetShape()[SHAPE_DIMENSION_TWO] == SHAPE_DIMENSION_THREE)
    {
        widthExtend = RGB_WIDTH_EXTEND;
        outputImageDesc.picture_format = static_cast<hi_pixel_format>(PIXEL_FORMAT_BGR_888);
        outputImageDesc.picture_width_stride =
            DVPP_ALIGN_UP(outputImageDesc.picture_width, DVPP_ALIGN_WIDTH) * SHAPE_DIMENSION_THREE;
    }
    else
    {
        widthExtend = GRAY_WIDTH_EXTEND;
    ...
    outputImageDesc.picture_buffer_size = outputImageDesc.picture_width_stride * outputImageDesc.picture_height_stride;
...
static APP_ERROR SetResizePasteInputPicsDesc(std::vector<MxBase::Tensor> &inputPics, hi_vpc_pic_info **batchInputPic,
                                             uint32_t *regionCount, bool *reuseInputVec, uint32_t numPastedImage,
                                             int32_t deviceId, hi_vpc_chn chnId)
{
    APP_ERROR ret = APP_ERR_OK;
                                    ...
                                    std::vector<MxBase::Rect> &pasteRects, MxBase::Tensor &dst, bool keepMargin)
{
    ...
    ret = SetResizePasteInputPicsDesc(inputPics, batchInputPic, regionCount, reuseInputVec, numPastedImage,
                                      background.GetDeviceId(), chnId);
    if (ret != APP_ERR_OK)
    {
    ...
    ret = DoVpcResizePaste(background, chnId, batchInputPic, numPastedImage, cropResizePasteInfos, regionCount);
    if (ret != APP_ERR_OK)
    {
        FreeDataForResizePaste(batchInputPic, reuseInputVec, outputPic, reuseOutputTensor, numPastedImage);
        for (size_t j = 0; j < numPastedImage; j++)
...
void CallBack::ResizePasteCallBackFunc(void *arg)
{
    ...
    ret = ResizePasteProcess(input->background, input->inputPics, input->pasteRects, input->dst, input->keepMargin);
    delete input;
    if (ret != APP_ERR_OK)
    {
...
{
    APP_ERROR ret =
        aclrtLaunchCallback(ResizePasteCallBackFunc, static_cast<void *>(input), ACL_CALLBACK_BLOCK, stream.stream);
    if (ret != APP_ERR_OK)
    {
        delete input;
                      ...
                      MxBase::AscendStream &stream)
{
    ...
    {
        CallBack dvppCallback;
        auto *resizePasteCallbackParam =
            new ResizePasteCallbackParam{background, inputPics, pasteRects, dst, keepMargin};
...
static APP_ERROR CheckNHWCTensorShape(const Tensor &src, std::string tensorName)
{
    ...
    {
        LogError << "CheckWarpOpSrcDstShapeParam: The " << tensorName << " is invalid, N must in [" << MIN_WARPOP_N
                 << ", " << MAX_WARPOP_N << "], but N(" << srcShape[SHAPE_DIMENSION_ZERO] << ") please check!"
                 << GetErrorInfo(APP_ERR_COMM_INVALID_PARAM);
        return APP_ERR_COMM_INVALID_PARAM;
    }
    ...
    if (srcShape[SHAPE_DIMENSION_ONE] < MIN_WARPOP_HEIGHT || srcShape[SHAPE_DIMENSION_ONE] > MAX_WARPOP_HEIGHT)
    {
        LogError << "CheckWarpOpSrcDstShapeParam: The " << tensorName << " is invalid, and height must in ["
                 << MIN_WARPOP_HEIGHT << ", " << MAX_WARPOP_HEIGHT << "], but height(" << srcShape[SHAPE_DIMENSION_ONE]
    ...
    if (srcShape[SHAPE_DIMENSION_TWO] < MIN_WARPOP_WIDTH || srcShape[SHAPE_DIMENSION_TWO] > MAX_WARPOP_WIDTH)
    {
        LogError << "CheckWarpOpSrcDstShapeParam: The " << tensorName << " is invalid, and width must in ["
                 << MIN_WARPOP_WIDTH << ", " << MAX_WARPOP_WIDTH << "], but width(" << srcShape[SHAPE_DIMENSION_TWO]
                 << ") please check!" << GetErrorInfo(APP_ERR_COMM_INVALID_PARAM);
        return APP_ERR_COMM_INVALID_PARAM;
    }
...
static APP_ERROR CheckWarpOpSrcDstShapeParam(const Tensor &src, const Tensor &dst, int)
{
    ...
    {
        LogError << "The src tensor device(" << src.GetDeviceId() << ") and the dst tensor device(" << dst.GetDeviceId()
...
static APP_ERROR CheckWarpOpParam(const Tensor &src, const Tensor &dst,
                                  const std::vector<std::vector<float>> &transMatrix, const WarpParams &warpParams,
                                  AscendStream &stream)
{
    APP_ERROR ret = CheckWarpOpSrcDstParam(src, dst, warpParams.warpType, stream);
    if (ret != APP_ERR_OK)
    {
        LogError << "CheckWarpOpParam: The src and dst param check failed, please check!" << GetErrorInfo(ret);
        return ret;
    }
        ...
        warpParams.borderValue > MAX_WARPOP_BORDER_VALUE + EPSILON)
    {
        LogError << "CheckWarpOpParam: BorderValue must in [" << MIN_WARPOP_BORDER_VALUE << ", "
                 << MAX_WARPOP_BORDER_VALUE << "], but get(" << warpParams.borderValue << "), please check!"
                 << GetErrorInfo(APP_ERR_COMM_INVALID_PARAM);
        return APP_ERR_COMM_INVALID_PARAM;
    }
...
static APP_ERROR GetWarpPerspectiveFP32TillingParam(int, const aclTensorDesc *const[], int32_t,
...
static APP_ERROR GetWarpPerspectiveFP16TillingParam(int, const aclTensorDesc *const[], int32_t,
                                                    const aclTensorDesc *const[], const aclopAttr *,
                                                    aclopKernelDesc *aclopKernelDesc)
{
    int32_t param = 0;
...
static APP_ERROR GetWarpPerspectiveU8TillingParam(int, const aclTensorDesc *const[], int32_t,
                                                  const aclTensorDesc *const[], const aclopAttr *,
                                                  aclopKernelDesc *aclopKernelDesc)
{
    int32_t param = 0;
                                                 ...
                                                 const WarpPerspectiveMode warpPerspectiveMode)
{
    ...
    int coreNum =
        std::min(static_cast<int>((c * h * w + AI_CORE_NUMS_310P - 1) / AI_CORE_NUMS_310P), AI_CORE_NUMS_310P);
    ...
    std::vector<float> outShape{inverseMat[0][0],
                                inverseMat[0][1],
                                inverseMat[0][2],
                                inverseMat[1][0],
                                inverseMat[1][1],
                                ...
                                static_cast<float>(h),
                                static_cast<float>(w),
                                static_cast<float>(warpPerspectiveMode),
                                static_cast<float>(paddingMode),
                                borderValue,
                                borderValue,
                                borderValue,
                                borderValue,
...
static std::vector<SingleOperator> GetWarpPerspectiveSingleOperator(const Tensor &src)
{
    ...
    if (src.GetDataType() == TensorDType::UINT8)
    {
        op = {SingleOperator{"warp_perspective_uint8", "warp_perspective_uint8.o", "warp_perspective_uint8__kernel0",
                             "warp_perspective_uint8__kernel0", ENGINE_TYPE, GetWarpPerspectiveU8TillingParam}};
    }
    ...
    {
        LogError << "WarpPerspective: The src/dst tensor's dataType must be the same, and only support fp16 fp32 or u8."
                 << GetErrorInfo(APP_ERR_COMM_INVALID_PARAM);
...
APP_ERROR WarpPerspective(const Tensor &src, Tensor &dst, const std::vector<std::vector<float>> transMatrix,
                          const PaddingMode paddingMode, const float borderValue,
                          const WarpPerspectiveMode warpPerspectiveMode, AscendStream &stream)
{
    ...
    Tensor tilingTensor;
    ret =
        GetWarpPerspectiveTillingTensor(src, tilingTensor, transMatrix, paddingMode, borderValue, warpPerspectiveMode);
    if (ret != APP_ERR_OK)
    {
...
static APP_ERROR WarpAffineTilling(int, const aclTensorDesc *const inputDesc[], int32_t, const aclTensorDesc *const[],
...
static APP_ERROR GetWarpAffineTillingTensor(const Tensor &src, const Tensor &dstTensor, Tensor &tillingTensor,
                                            std::vector<std::vector<float>> mat, WarpParams &warpParams,
                                            bool isRotate = false)
{
    ...
    uint32_t channel = isRotate
                           ? (shapeSrc.size() == HWC_SHAPE_SIZE ? shapeSrc[SHAPE_DIMENSION_TWO] : MIN_WARPOP_CHANNEL)
                           : shapeSrc[SHAPE_DIMENSION_THREE];
    dst[WARP_PARAM_HEIGHT] =
        isRotate ? dstTensor.GetShape()[SHAPE_DIMENSION_ZERO] : dstTensor.GetShape()[SHAPE_DIMENSION_ONE];
    dst[WARP_PARAM_WIDTH] =
        isRotate ? dstTensor.GetShape()[SHAPE_DIMENSION_ONE] : dstTensor.GetShape()[SHAPE_DIMENSION_TWO];
    ...
    dst[WARP_PARAM_CORE] = (height > minSize || width > minSize)
                               ? coreNum
                               : std::min((channel * height * width + coreNum - 1) / coreNum, coreNum);
...
void SetRotateOffsetHeightAndWidth(const Tensor &src, uint32_t &offsetHeight, uint32_t &offsetWidth, uint32_t,
                                   uint32_t perVarNum)
{
...
APP_ERROR AccelerateForRotate(const Tensor &src, Tensor &dst, const RotateAngle angle, AscendStream &stream,
                              bool isConvert)
{
...
static APP_ERROR CheckCropRoi(const Tensor &inputTensor, const Rect &cropRect)
{
    if (inputTensor.IsWithMargin())
    {
        ...
        if (cropRect.x1 > roiRect.x1 || cropRect.y1 > roiRect.y1 || cropRect.x0 > roiRect.x1 ||
            cropRect.y0 > roiRect.y1)
        {
            LogError << "The cropRect should inside the inputTensor ROI zone, cropRect is {" << cropRect.x0 << ", "
                     << cropRect.y0 << ", " << cropRect.x1 << ", " << cropRect.y1 << "}, tensor roi is {" << roiRect.x0
                     << ", " << roiRect.y0 << ", " << roiRect.x1 << ", " << roiRect.y1 << "}."
                     << GetErrorInfo(APP_ERR_COMM_INVALID_PARAM);
            return APP_ERR_COMM_INVALID_PARAM;
        }
    }
                                  ...
                                  const std::vector<Size> &sizeVec, std::vector<Tensor> &outputTensorVec)
{
    ...
    for (size_t i = 0; i < outputTensorVec.size(); i++)
    {
        ...
        {
            LogError << "CropResize: Unsupported outputTensor Datatype, Only support uint8 TensorDType, Index: " << i
                     << "." << GetErrorInfo(APP_ERR_COMM_INVALID_PARAM);
            return APP_ERR_COMM_INVALID_PARAM;
        }
                              ...
                              std::vector<Tensor> &outputTensorVec, bool keepMargin)
{
    ...
    for (size_t i = 0; i < sizeVec.size(); ++i)
    {
        ...
        if (ret != APP_ERR_OK)
        {
            LogError << "Failed to Set OutputTensor in CropResize operation, index: " << i << "." << GetErrorInfo(ret);
            return ret;
        }
    }
    return ret;
                                   ...
                                   const Tensor &outputTensor, bool &reuseOutputTensor)
{
    ...
    crop_resize_info.dest_pic_info.picture_buffer_size =
        crop_resize_info.dest_pic_info.picture_height_stride * crop_resize_info.dest_pic_info.picture_width_stride;
...
void SetCropRegionAndResizeInfo(hi_vpc_crop_resize_region &crop_resize_info, const Rect &cropRect, const Size &size,
                                const Interpolation interpolation)
{
...
APP_ERROR DoVpcCropResize(hi_vpc_crop_resize_region *crop_resize_infos, hi_vpc_pic_info &inputDesc, hi_vpc_chn &chnId,
                          std::vector<Tensor> &outputTensorVec, bool keepMargin)
{
    APP_ERROR ret = APP_ERR_OK;
    hi_u32 taskId = 0;
    ...
    for (size_t i = 0; i < outputTensorVec.size(); ++i)
    {
        if (!keepMargin && !IsMemoryReusable(outputTensorVec[i]))
        {
            ...
            if (ret != APP_ERR_OK)
            {
                LogError << "Failed to copy dvpp memory for CropResize output tensor, index: " << i << "."
                         << GetErrorInfo(ret);
                return ret;
            }
        }
                            ...
                            const Interpolation interpolation, bool keepMargin, AscendStream &)
{
    ...
    for (size_t i = 0; i < cropRectVec.size(); ++i)
    {
        ...
        if (ret != APP_ERR_OK)
        {
            FreeDataForCropResize(inputDesc, crop_resize_infos, reuseInputTensor, reuseOutputTensorVec, i);
    ...
    if (ret != APP_ERR_OK)
    {
        ...
        FreeDataForCropResize(inputDesc, crop_resize_infos, reuseInputTensor, reuseOutputTensorVec,
                              outputTensorVec.size() - 1);
        return ret;
    }
...
void CallBack::CropResizeCallBackFunc(void *arg)
{
    ...
    ret = CropResizeProcess(input->src, input->rectVec, input->sizeVec, input->dst, input->interpolation,
                            input->keepMargin, input->stream);
    delete input;
    if (ret != APP_ERR_OK)
    {
        LogError << "Failed to exec CropResizeCallBackFunc" << GetErrorInfo(ret);
...
APP_ERROR CropResize(const Tensor &inputTensor, const std::vector<Rect> &cropRectVec, const std::vector<Size> &sizeVec,
                     std::vector<Tensor> &outputTensorVec, const Interpolation interpolation, bool keepMargin,
                     AscendStream &stream)
{
    APP_ERROR ret = APP_ERR_OK;
                            ...
                            uint32_t channel, bool &reuseOutputTensor)
{
    ...
    else
    {
        ...
        if (ret != APP_ERR_OK)
        {
            LogError << "Failed to malloc dvpp memory for output tensor." << GetErrorInfo(ret, "hi_mpi_dvpp_malloc");
            return APP_ERR_COMM_ALLOC_MEM;
        }
    }
    return APP_ERR_OK;
...
APP_ERROR DoVpcCrop(hi_vpc_chn &chnId, hi_vpc_pic_info &cropInputDesc, hi_vpc_pic_info &cropOutputDesc,
                    const Rect &cropRect)
{
    APP_ERROR ret = APP_ERR_OK;
...
APP_ERROR DoCropWithHimpi(hi_vpc_chn &chnId, const Tensor &inputTensor, const Rect &cropRect, Tensor &outputTensor,
                          bool keepMargin)
{
    uint32_t channel = GetWidthExtend(inputTensor);
...
APP_ERROR CropProcess(const Tensor &inputTensor, const Rect &cropRect, Tensor &outputTensor, bool keepMargin,
                      AscendStream &)
{
    uint32_t height = 0;
    uint32_t width = 0;
...
APP_ERROR Crop(const Tensor &inputTensor, const Rect &cropRect, Tensor &outputTensor, bool keepMargin,
               AscendStream &stream)
{
    ...
    {
        CallBack dvppCallback;
        CropCallbackParam *cropCallbackParam =
            new CropCallbackParam{inputTensor, cropRect, outputTensor, keepMargin, stream};
...
void FreeDataForBatchCrop(hi_vpc_pic_info &inputDesc, hi_vpc_crop_region_info *cropRegionInfos, bool reuseInputTensor,
                          bool *reuseOutputTensorVec, size_t currIndex)
{
    if (!reuseInputTensor && inputDesc.picture_address != nullptr)
    {
        DVPPMemoryFreeFunc(inputDesc.picture_address);
                                 ...
                                 bool *reuseOutputTensorVec)
{
    for (size_t i = 0; i < outputTensorVec.size(); ++i)
    {
        ...
        if (SetCropOutputDesc(cropRegionInfos[i].dest_pic_info, outputTensorVec[i], cropRectVec[i], channel,
                              reuseOutputTensorVec[i]) != APP_ERR_OK)
        {
            return APP_ERR_COMM_INVALID_PARAM;
        }
        uint32_t height = 0;
                         ...
                         std::vector<Tensor> &outputTensorVec, bool keepMargin)
{
    ...
    for (size_t i = 0; i < outputTensorVec.size(); ++i)
    {
        if (!keepMargin && !IsMemoryReusable(outputTensorVec[i]))
        {
            ...
            if (ret != APP_ERR_OK)
            {
                LogError << "Failed to copy dvpp memory for BatchCrop output tensor, index: " << i << "."
                         << GetErrorInfo(ret);
                FreeDataForBatchCrop(inputDesc, cropRegionInfos, reuseInputTensor, reuseOutputTensorVec, count);
                return ret;
            }
                             ...
                             std::vector<Tensor> &outputTensorVec, bool keepMargin)
{
    ...
    for (size_t i = 0; i < cropRectVec.size(); ++i)
    {
        ...
        if (ret != APP_ERR_OK)
        {
            LogError << "Failed to Set OutputTensor in BatchCrop operation, index: " << i << "." << GetErrorInfo(ret);
            return ret;
        }
    }
    return ret;
               ...
               bool keepMargin, AscendStream &stream)
{
    ...
    {
        CallBack dvppCallback;
        BatchCropCallbackParam *batchCropCallbackParam =
            new BatchCropCallbackParam{inputTensor, cropRectVec, outputTensorVec, keepMargin, stream};
