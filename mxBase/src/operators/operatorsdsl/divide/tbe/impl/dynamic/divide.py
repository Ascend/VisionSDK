...
@register_op_compute("divide")
def divide_compute(x1, x2, y, kernel_name="divide"):
    """
    ...
    if dtype == "uint8":
        ...
        x1 = tbe.cast_to(x1, dtype)
        x2 = tbe.cast_to(x2, dtype)
...
def divide(x1, x2, y, kernel_name="divide"):
    """
    To do: Implement the operator by referring to the
           TBE Operator Development Guide.
    """
    ...
    for (_x1, _x2) in ins:
        with tbe.compute():
            ...
            # 3. compute divide
            res = divide_compute(data_x1, data_x2, y, kernel_name)
