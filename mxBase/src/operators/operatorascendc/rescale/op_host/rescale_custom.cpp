...
* -------------------------------------------------------------------------
 * Description: rescale operator host file.
 * Author: MindX SDK
 * Create: 2024
 * History: NA
 */
#include "rescale_custom_tiling.h"
...
namespace optiling {
    ...
    static ge::graphStatus TilingFunc(gert::TilingContext *context)
    {
        ...
        const float* bias = attrs->GetAttrPointer<float>(1);
...
namespace ops {
    class RescaleCustom : public OpDef {
    public:
        explicit RescaleCustom(const char* name) : OpDef(name)
        {
            this->Input("x")
                .ParamType(REQUIRED)
            ...
            this->Attr("bias").AttrType(REQUIRED).Float();
