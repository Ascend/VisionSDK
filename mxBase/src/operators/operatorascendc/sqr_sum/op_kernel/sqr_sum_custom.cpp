...
* -------------------------------------------------------------------------
 * Description: sqrSum operator kernel file.
 * Author: MindX SDK
 * Create: 2024
 * History: NA
 */
...
template <typename T> class KernelSqrSum {
public:
    __aicore__ inline KernelSqrSum(size_t channelNum) : chnNum(channelNum) {}
                                ...
                                size_t gmIdx, size_t gmDataLen, size_t tilingDataSize)
    {
        ...
        pipe.InitBuffer(inQueueZero, bufferNum, bufferBytes);
        ...
        // set zero to the zGM
        auto zeroLocal = inQueueZero.AllocTensor<float>();
        Duplicate(zeroLocal, INIT_VAL, ZGM_LEN);
        event_t eventIDVToMTE3 = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::V_MTE3));
        set_flag(PIPE_V, PIPE_MTE3, eventIDVToMTE3);
        wait_flag(PIPE_V, PIPE_MTE3, eventIDVToMTE3);
        DataCopy(zGm, zeroLocal, ZGM_LEN);
        inQueueZero.FreeTensor(zeroLocal);
    }
    ...
    __aicore__ inline void ComputeC3(uint32_t idx, size_t len, int64_t blockIndex, LocalTensor<float> xLocal,
                                     LocalTensor<float> tmpLocal, LocalTensor<float> workLocal,
...
protected:
    ...
    TQue<QuePosition::VECIN, BUFFER_NUM> inQueueZero;
...
extern "C" __global__ __aicore__ void sqr_sum_custom(GM_ADDR x, GM_ADDR z, GM_ADDR workspace, GM_ADDR tiling)
{
    GET_TILING_DATA(tilingData, tiling);
    if (TILING_KEY_IS(1)) {
...
void sqr_sum_custom_do(uint32_t blockDim, void *l2ctrl, void *stream, uint8_t *x, uint8_t *z, uint8_t *workspace,
                       uint8_t *tiling)
{
    sqr_sum_custom<<<blockDim, l2ctrl, stream>>>(x, z, workspace, tiling);
}
#endif