...
* -------------------------------------------------------------------------
 * Description: min operator kernel file.
 * Author: MindX SDK
 * Create: 2024
 * History: NA
 */
...
public:
    ...
    __aicore__ void CalcForAlign32(uint32_t idx, size_t len)
    {
        ...
        if constexpr(sizeof(T) == 1) {
            ...
            Min(txLocal, txLocal, tyLocal, len);
            Cast(zLocal, txLocal, RoundMode::CAST_NONE, len);
        } else {
            Min(zLocal, xLocal, yLocal, len);
        }
...
extern "C" __global__ __aicore__ void min_custom(GM_ADDR x, GM_ADDR y, GM_ADDR z, GM_ADDR workspace, GM_ADDR tiling)
{
    GET_TILING_DATA(tilingData, tiling);
    if (TILING_KEY_IS(1)) {
...
void min_custom_do(uint32_t blockDim, void *l2ctrl, void *stream, uint8_t *x, uint8_t *y, uint8_t *z,
                   uint8_t *workspace, uint8_t *tiling)
{
    min_custom<<<blockDim, l2ctrl, stream>>>(x, y, z, workspace, tiling);
}
