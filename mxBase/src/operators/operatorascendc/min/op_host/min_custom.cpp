...
* -------------------------------------------------------------------------
 * Description: min operator host file.
 * Author: MindX SDK
 * Create: 2024
 * History: NA
 */

#include "min_custom_tiling.h"
...
namespace ops {
    class MinCustom : public OpDef {
    public:
        explicit MinCustom(const char* name) : OpDef(name)
        {
            this->Input("x")
                .ParamType(REQUIRED)
