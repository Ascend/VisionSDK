    ...
    __aicore__ inline void CalcForAlign32(uint32_t idx, size_t h, CalcWindow calcWindow)
    {
        ...
        if constexpr (sizeof(T) == 1)
        {
            ...
            if (calcWindow.isTail)
            {
                for (size_t i = 0; i < h; i++)
                {
                    Gather(dstCalc[i * calcWindow.dstWidth], srcCalc[calcWindow.srcWidth * i], tailOffset, 0,
                           calcWindow.dstWidth);
                    pipe_barrier(PIPE_ALL);
                }
            }
        ...
        else
        {
            if (calcWindow.isTail)
            {
                for (size_t i = 0; i < h; i++)
                {
                    Gather(dstLocal[i * calcWindow.dstWidth], srcLocal[calcWindow.srcWidth * i], tailOffset, 0,
                           calcWindow.dstWidth);
                    pipe_barrier(PIPE_ALL);
                }
            }
