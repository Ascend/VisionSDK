...
* -------------------------------------------------------------------------
 * Description: pow operator kernel file.
 * Author: Vision SDK
 * Create: 2025
 * History: NA
 */
...
public:
                                ...
                                size_t gmIdx, size_t gmDataLen)
    {
        ...
        if (sizeof(T) == 1) {
            ...
            pipe.InitBuffer(bufYf16, bufferBytes * sizeof(half));
        }
...
protected:
    ...
    TBuf<TPosition::VECCALC> bufYf16;
...
extern "C" __global__ __aicore__ void pow_custom(GM_ADDR x, GM_ADDR y, GM_ADDR z, GM_ADDR workspace,
                                                 GM_ADDR tiling)
{
    if (TILING_KEY_IS(1)) {
        constexpr float ubVarNum = 3 * 2;
        run_op<half>(x, y, z, tiling, ubVarNum);
    } else if (TILING_KEY_IS(2)) {
        constexpr float ubVarNum = 3 * 2;
        run_op<float>(x, y, z, tiling, ubVarNum);
    } else if (TILING_KEY_IS(3)) {
        constexpr float ubVarNum = (1 * 3 + 2 * 2) * 2;
...
void pow_custom_do(uint32_t blockDim, void *l2ctrl, void *stream, uint8_t *x, uint8_t *y, uint8_t *z,
                   uint8_t *workspace, uint8_t *tiling)
{
    pow_custom<<<blockDim, l2ctrl, stream>>>(x, y, z, workspace, tiling);
}
