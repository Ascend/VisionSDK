...
constexpr uint64_t HWC_SHAPE_SIZE = 3;
static ge::graphStatus TilingFunc(gert::TilingContext *context)
{
    if (context == nullptr) {
        return ge::GRAPH_FAILED;
    }
    TilingDataSum tiling;
    auto tensorX = context->GetInputTensor(0);
    if (tensorX == nullptr) {
        return ge::GRAPH_FAILED;
    }
    uint32_t totalLength = tensorX->GetShapeSize();
    tiling.set_totalSize(totalLength);
    auto shape = tensorX->GetShape().GetOriginShape();
    auto dimNum = shape.GetDimNum();
    if (dimNum < HWC_SHAPE_SIZE) {
        return ge::GRAPH_FAILED;
    }
    uint32_t batchNum = dimNum == HWC_SHAPE_SIZE ? 1 : shape[0];
    tiling.set_batchNum(batchNum);
    tiling.set_chnNum(shape[dimNum - 1]);
