...
* -------------------------------------------------------------------------
 * Description: max operator kernel file.
 * Author: MindX SDK
 * Create: 2024
 * History: NA
 */
...
public:
    ...
    __aicore__ void CalcForAlign32(uint32_t idx, size_t len)
    {
        ...
        if constexpr(sizeof(T) == 1) {
            ...
            Max(txLocal, txLocal, tyLocal, len);

            Cast(zLocal, txLocal, RoundMode::CAST_NONE, len);
        } else {
            Max(zLocal, xLocal, yLocal, len);
        }
...
void max_custom_do(uint32_t blockDim, void *l2ctrl, void *stream, uint8_t *x, uint8_t *y, uint8_t *z,
                   uint8_t *workspace, uint8_t *tiling)
{
    max_custom<<<blockDim, l2ctrl, stream>>>(x, y, z, workspace, tiling);
}
