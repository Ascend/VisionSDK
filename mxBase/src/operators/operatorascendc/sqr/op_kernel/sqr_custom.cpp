...
* -------------------------------------------------------------------------
 * Description: sqr operator kernel file.
 * Author: MindX SDK
 * Create: 2024
 * History: NA
 */
...
public:
    ...
    __aicore__ void CalcForAlign32(uint32_t idx, size_t len)
    {
        ...
        if constexpr(sizeof(T) == 1) {
            ...
            Mul(txLocal, txLocal, txLocal, len);
...
extern "C" __global__ __aicore__ void sqr_custom(GM_ADDR x, GM_ADDR z, GM_ADDR workspace, GM_ADDR tiling) {
    if (TILING_KEY_IS(1)) {
...
void sqr_custom_do(uint32_t blockDim, void *l2ctrl, void *stream, uint8_t *x, uint8_t *z,
                   uint8_t *workspace, uint8_t* tiling)
{
    sqr_custom<<<blockDim, l2ctrl, stream>>>(x, z, workspace, tiling);
}
#endif
