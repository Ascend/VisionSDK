...
* -------------------------------------------------------------------------
 * Description: scaleadd operator kernel file.
 * Author: MindX SDK
 * Create: 2024
 * History: NA
 */
...
template <typename T>
class KernelScaleAdd {
public:
    __aicore__ inline KernelScaleAdd(float scale): scale(scale)
    {}
    ...
    __aicore__ inline void CalcForAlign32(uint32_t idx, size_t len)
    {
        ...
        if constexpr(sizeof(T) == 1) {
            ...
            Duplicate<half>(tyLocal, static_cast<half>(this->scale), len);
            Mul(txLocal, txLocal, tyLocal, len);
            Cast(tyLocal, yLocal, RoundMode::CAST_NONE, len);
            Add(txLocal, txLocal, tyLocal, len);

            Cast(zLocal, txLocal, RoundMode::CAST_TRUNC, len);
        } else {
            // scaleAdd
            Duplicate<T>(zLocal, static_cast<T>(this->scale), len);
            Mul(zLocal, xLocal, zLocal, len);
            Add(zLocal, zLocal, yLocal, len);
        }
...
extern "C" __global__ __aicore__ void scale_add_custom(GM_ADDR x, GM_ADDR y, GM_ADDR z, GM_ADDR workspace,
                                                       GM_ADDR tiling)
{
    if (TILING_KEY_IS(1)) {
...
void scale_add_custom_do(uint32_t blockDim, void *l2ctrl, void *stream, uint8_t *x, uint8_t *y, uint8_t *z,
                         uint8_t *workspace, uint8_t *tiling)
{
    scale_add_custom<<<blockDim, l2ctrl, stream>>>(x, y, z, workspace, tiling);
}
