...
* -------------------------------------------------------------------------
 * Description: compare operator host file.
 * Author: MindX SDK
 * Create: 2024
 * History: NA
 */
#include "compare_custom_tiling.h"
...
namespace optiling {
    ...
    static ge::graphStatus TilingFunc(gert::TilingContext *context)
    {
        ...
        const uint32_t* operation = attrs->GetAttrPointer<uint32_t>(0);
...
namespace ops {
    class CompareCustom : public OpDef {
    public:
        explicit CompareCustom(const char* name) : OpDef(name)
        {
            this->Input("x")
                .ParamType(REQUIRED)
