...
* -------------------------------------------------------------------------
 * Description: compare operator kernel file.
 * Author: MindX SDK
 * Create: 2024
 * History: NA
 */
...
class KernelCompare {
public:
    __aicore__ inline KernelCompare(CMPMODE cmpMode, float trueVal = 255, float falseVal = 0)
        : cmpMode(cmpMode), trueVal(trueVal), falseVal(falseVal)
    {}
...
extern "C" __global__ __aicore__ void compare_custom(GM_ADDR x, GM_ADDR y, GM_ADDR z, GM_ADDR workspace, GM_ADDR tiling)
{
    if (TILING_KEY_IS(1)) {
...
void compare_custom_do(uint32_t blockDim, void *l2ctrl, void *stream, uint8_t *x, uint8_t *y, uint8_t *z,
                       uint8_t *workspace, uint8_t *tiling)
{
    compare_custom<<<blockDim, l2ctrl, stream>>>(x, y, z, workspace, tiling);
}
#endif
