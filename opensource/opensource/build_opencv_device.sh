...
# Build
fileName="opencv_device"
packageFQDN="opencv/opencv-device@4.11.0"
packageName="opencv"
cd "$fileName" || {
...
cmake \
  ...
  -DCMAKE_INSTALL_PREFIX="$(pwd)/../../tmp/$packageName-device" \
  -DCMAKE_INSTALL_LIBDIR="$(pwd)/../../tmp/$packageName-device/lib" \
  -DCMAKE_TOOLCHAIN_FILE="$(pwd)/../../Ascend.cmake" \
  -DCMAKE_SKIP_RPATH=TRUE \
  -DCMAKE_BUILD_TYPE=RELEASE \
  -DBUILD_INFO_SKIP_TIMESTAMP=ON \
  -DCMAKE_CXX_FLAGS_RELEASE="-O2 -DNDEBUG -std=c++11 -fPIE -fstack-protector-all -fPIC -Wall" \
