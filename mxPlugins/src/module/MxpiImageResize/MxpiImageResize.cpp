...
APP_ERROR MxpiImageResize::Init(std::map<std::string, std::shared_ptr<void>>& configParamMap)
{
    ...
    if ((resizeType_ == RESIZETYPE["Resizer_KeepAspectRatio_FastRCNN"] ||
        ...
        paddingType_ == PADDINGTYPE["Padding_NO"]) {
...
APP_ERROR MxpiImageResize::CheckOutputImage(const uint32_t width, const uint32_t height)
{
    ...
    if (width < OUTPUT_MIN_WIDTH || width > OUTPUT_MAX_WIDTH) {
        errorInfo_ << "Output width(" << width << ") of the image is illegal, not in the range [" << OUTPUT_MIN_WIDTH
                   << "," << OUTPUT_MAX_WIDTH << "], failed." << GetErrorInfo(APP_ERR_DVPP_INVALID_IMAGE_WIDTH);
        LogError << errorInfo_.str();
        return APP_ERR_DVPP_INVALID_IMAGE_WIDTH;
    }
    if (height < OUTPUT_MIN_HEIGHT || height > OUTPUT_MAX_HEIGHT) {
        errorInfo_ << "Output height(" << height << ") of the image is illegal, not in the range [" << OUTPUT_MIN_HEIGHT
                   << "," << OUTPUT_MAX_HEIGHT << "], failed." << GetErrorInfo(APP_ERR_DVPP_INVALID_IMAGE_HEIGHT);
        LogError << errorInfo_.str();
                                      ...
                                      MxpiVisionInfo& srcMxpiVisionInfo, MxpiVisionData& srcMxpiVisionData)
{
    ...
    if (resizeType_ == RESIZETYPE["Resizer_KeepAspectRatio_FastRCNN"] ||
        resizeType_ == RESIZETYPE["Resizer_KeepAspectRatio_Fit"] || resizeType_ == RESIZETYPE["Resizer_OnlyPadding"]) {
        ...
        cropResizePasteConfig_ = {0,
                                  dvppInputDataInfo_.width,
                                  0,
                                  dvppInputDataInfo_.height,
                                  0,
                                  dvppOutputDataInfo_.width,
                                  0,
                                  dvppOutputDataInfo_.height,
                                  resizeConfig_.interpolation};
    }
...
void MxpiImageResize::OpencvResizeProcess(cv::Mat& resizeRGB, cv::Mat& imageRGB, MxpiVisionInfo& srcMxpiVisionInfo)
{
    ...
    if (flagValue) {
        LogError << "SrcMxpiVisionInfo.width(): " << srcMxpiVisionInfo.width()
                 << ", srcMxpiVisionInfo.height(): " << srcMxpiVisionInfo.height() << "must not equal to zero!"
                 << GetErrorInfo(APP_ERR_COMM_INVALID_PARAM);
        return;
    }
...
APP_ERROR MxpiImageResize::OpencvPaddingProcess(cv::Mat& imageRGB, MxpiVisionInfo& srcMxpiVisionInfo)
{
    ...
    if (paddingType_ == PADDINGTYPE["Padding_RightDown"]) {
        ...
        if (padTop >= 0 || padLeft >= 0) {
            cv::Scalar value;
            GetScalarValue(srcMxpiVisionInfo, value);
            cv::copyMakeBorder(imageRGB, imageRGB, padTop, padBottom, padLeft, padRight, cv::BORDER_CONSTANT, value);
        }
    }
    return APP_ERR_OK;
}
...
{
    if (resizeType_ != RESIZETYPE["Resizer_Stretch"] && resizeType_ != RESIZETYPE["Resizer_KeepAspectRatio_Short"] &&
        resizeType_ != RESIZETYPE["Resizer_KeepAspectRatio_Long"] && resizeType_ != RESIZETYPE["Resizer_Rescale"] &&
        resizeType_ != RESIZETYPE["Resizer_OnlyPadding"] && resizeType_ != RESIZETYPE["Resizer_Rescale_Double"] &&
        resizeType_ != RESIZETYPE["Resizer_MS_Yolov4"] && resizeType_ != RESIZETYPE["Resizer_PaddleOCR"] &&
...
APP_ERROR MxpiImageResize::CheckOpencvParam(MxpiVisionInfo& srcMxpiVisionInfo, MxpiVisionData& srcMxpiVisionData)
{
    ...
    if (srcMxpiVisionInfo.width() == 0 || srcMxpiVisionInfo.height() == 0) {
        errorInfo_ << "Input picture width or height is zero, srcMxpiVisionInfo.width=" << srcMxpiVisionInfo.width()
                   << "srcMxpiVisionInfo.height=" << srcMxpiVisionInfo.height() << "."
                   << GetErrorInfo(APP_ERR_MXPLUGINS_MEMEROY_TYPE_MISMATCH);
        LogError << errorInfo_.str();
        return APP_ERR_MXPLUGINS_MEMEROY_TYPE_MISMATCH;
    }
                                         ...
                                         cv::Mat& resizeRGB)
{
    ...
    if (srcMxpiVisionData.datatype() == MxTools::MXPI_DATA_TYPE_UINT8) {
        outputDataInfo.dataSize =
            static_cast<uint32_t>(resizeRGB.cols) * static_cast<uint32_t>(resizeRGB.rows) * YUV444_RGB_WIDTH_NU;
...
void MxpiImageResize::CalcMSPasteArea(CropRoiConfig& pasteConfig, uint32_t& dataSize)
{
    ...
    if (IsDenominatorZero(dvppInputDataInfo_.width) || IsDenominatorZero(dvppInputDataInfo_.height)) {
        LogError << "DvppInputDataInfo_.width: " << dvppInputDataInfo_.width
                 << ", dvppInputDataInfo_.height: " << dvppInputDataInfo_.height << "must not equal to zero!"
                 << GetErrorInfo(APP_ERR_COMM_INVALID_PARAM);
        return;
    }
...
APP_ERROR MxpiImageResize::KeepAspectRatioResizer()
{
    ...
    cropResizePasteConfig_ = {cropConfig.x0,     cropConfig.x1 + 1,  cropConfig.y0,
                              cropConfig.y1 + 1, pasteConfig.x0,     pasteConfig.x1 + 1,
                              pasteConfig.y0,    pasteConfig.y1 + 1, 0};
    if (ret != APP_ERR_OK) {
        errorInfo_ << "Dvpp resize failed." << GetErrorInfo(APP_ERR_DVPP_RESIZE_FAIL);
        LogError << errorInfo_.str();
...
void MxpiImageResize::GetResizedOriginalSize(uint32_t& resizedOriginalWidth, uint32_t& resizedOriginalHeight) const
{
    if (resizeHeight_ * dvppInputDataInfo_.width > dvppInputDataInfo_.height * resizeWidth_) {
        ...
        resizedOriginalHeight =
            static_cast<uint32_t>(lround(dvppInputDataInfo_.height * resizeWidth_ * 1.0 / dvppInputDataInfo_.width));
    } else {
        resizedOriginalWidth =
            static_cast<uint32_t>(lround(dvppInputDataInfo_.width * resizeHeight_ * 1.0 / dvppInputDataInfo_.height));
                                                ...
                                                std::shared_ptr<MxTools::MxpiVisionList>& dstMxpiVisionListSptr)
{
    ...
    for (int i = 0; i < srcMxpiVisionListSptr->visionvec_size(); i++) {
        ...
        if (CheckPtrIsNullptr(dstMxpiVision, "dstMxpiVision"))
            return APP_ERR_COMM_ALLOC_MEM;
        MxpiMetaHeader* dstMxpiMetaHeaderList = dstMxpiVision->add_headervec();
        if (CheckPtrIsNullptr(dstMxpiMetaHeaderList, "dstMxpiMetaHeaderList"))
            return APP_ERR_COMM_ALLOC_MEM;
...
void MxpiImageResize::SetProperties(std::vector<std::shared_ptr<void>>& properties)
{
    ...
    auto scaleValue = std::make_shared<ElementProperty<uint>>(
        ElementProperty<uint>{UINT, "scaleValue", "scale value", "the scalaValue of resize image", 32, 32, 8192});
    auto paddingHeight = std::make_shared<ElementProperty<uint>>(
        ElementProperty<uint>{UINT, "paddingHeight", "height", "the height of resize image", 32, 32, 8192});
    auto paddingWidth = std::make_shared<ElementProperty<uint>>(
        ElementProperty<uint>{UINT, "paddingWidth", "width", "the width of resize image", 32, 32, 8192});
    auto paddingColorB = std::make_shared<ElementProperty<float>>(
        ElementProperty<float>{FLOAT, "paddingColorB", "paddingColorB", "the ColorB of resize image", 0, 0, 255});
    auto paddingColorG = std::make_shared<ElementProperty<float>>(
        ElementProperty<float>{FLOAT, "paddingColorG", "paddingColorG", "the ColorG of resize image", 0, 0, 255});
    auto paddingColorR = std::make_shared<ElementProperty<float>>(
        ElementProperty<float>{FLOAT, "paddingColorR", "paddingColorR", "the ColorR of resize image", 0, 0, 255});
    ...
    if (DeviceManager::IsAscend310() || DeviceManager::IsAscend310B()) {
        auto interpolation = std::make_shared<ElementProperty<int>>(
            ElementProperty<int>{INT, "interpolation", "interpolation", "set interpolation of resize image", 0, 0, 4});
        properties.push_back(interpolation);
    } else if (DeviceManager::IsAscend310P() || DeviceManager::IsAtlas800IA2()) {
        auto interpolation = std::make_shared<ElementProperty<int>>(
            ElementProperty<int>{INT, "interpolation", "interpolation", "set interpolation of resize image", 0, 0, 2});
        properties.push_back(interpolation);
    }
...
std::vector<std::shared_ptr<void>> MxpiImageResize::DefineProperties()
{
    ...
    auto parentNameProSptr = std::make_shared<ElementProperty<string>>(
        ElementProperty<string>{STRING, "parentName", "name", "the key of input data", "", "", ""});
    ...
    auto resizeHeightSptr = std::make_shared<ElementProperty<uint>>(
        ElementProperty<uint>{UINT, "resizeHeight", "height", "the height of resize image", 0, 0, OUTPUT_MAX_HEIGHT});
    auto resizeWidthSptr = std::make_shared<ElementProperty<uint>>(
        ElementProperty<uint>{UINT, "resizeWidth", "width", "the width of resize image", 0, 0, OUTPUT_MAX_WIDTH});
    ...
    auto paddingType = std::make_shared<ElementProperty<string>>(
        ElementProperty<string>{STRING, "paddingType", "type", "the padding mode of image", "Padding_NO", "", ""});

    std::vector<std::shared_ptr<void>> properties = {removeParentDataProSptr,
                                                     parentNameProSptr,
                                                     resizeHeightSptr,
                                                     resizeWidthSptr,
                                                     minDimension,
...
void MxpiImageResize::SetResizeConfig(MxpiVisionInfo& srcMxpiVisionInfo)
{
    ...
    LogDebug << "element(" << elementName_ << ") dynamicImageSize height=" << resizeConfig_.height
             << ", width=" << resizeConfig_.width;
}
