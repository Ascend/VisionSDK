...
* -------------------------------------------------------------------------
 * Description: MxpiPictureCropTest.
 * Author: MindX SDK
 * Create: 2021
 * History: NA
 */
...
namespace {
    class MxpiPictureCropTest : public testing::Test {
    public:
        virtual void SetUp()
        {
...
TEST_F(MxpiPictureCropTest, TestPictureCrop)
{
    ...
    APP_ERROR ret = mxStreamManager.CreateMultipleStreamsFromFile("picturecrop.pipeline");
    ...
    std::string streamName = "picturecrop";
    mxStreamManager.SendData(streamName, 0, mxstDataInput);
    sleep(2);

    mxStreamManager.DestroyAllStreams();
    std::string result = FileUtils::ReadFileContent("picturecrop.output");
