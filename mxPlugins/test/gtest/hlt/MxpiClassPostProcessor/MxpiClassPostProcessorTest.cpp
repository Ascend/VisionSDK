...
namespace {
    class MxpiClassPostProcessorTest : public testing::Test {
    public:
        virtual void SetUp()
        {
    ...
    TEST_F(MxpiClassPostProcessorTest, TestBasic)
    {
        LogInfo << "********case  TestMxpiClassPostProcessor********";
        std::string input = FileUtils::ReadFileContent("dog.jpg");
        MxStreamManager mxStreamManager;
        mxStreamManager.InitManager();
        APP_ERROR ret = mxStreamManager.CreateMultipleStreamsFromFile("ClassPostProcess.pipeline");
        ...
        std::string streamName = "classpostprocessor";
        mxStreamManager.SendData(streamName, 0, mxstDataInput);
        sleep(2);
        mxStreamManager.DestroyAllStreams();
        std::string result = FileUtils::ReadFileContent("classpostprocessor.output");
